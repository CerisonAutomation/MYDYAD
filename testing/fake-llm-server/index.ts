...
export function createFakeLlmApp(getPort: () => number) {
  ...
  app.get(
    "/api/default-approve-builds.txt",
  ...
  app.get("/api/mcp-catalog", (req: express.Request, res: express.Response) => {
  ...
  app.get("/ollama/api/tags", (req: express.Request, res: express.Response) => {
    const ollamaModels = {
      models: [
        {
          name: "testollama",
  ...
  app.get("/test-image.png", (_req: express.Request, res: express.Response) => {
    const tinyPngBase64 =
      "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==";
  ...
  app.post(
    "/engine/v1/sandboxes/:sandboxId/files",
  ...
  app.get(
    "/engine/v1/sandboxes/:sandboxId/status",
    (req: express.Request, res: express.Response) => {
      const sandbox = cloudSandboxes.get(req.params.sandboxId);
      if (!sandbox) {
        res.status(404).json({ error: "Sandbox not found" });
  ...
  app.post(
    "/engine/v1/sandboxes/:sandboxId/restart",
    (req: express.Request, res: express.Response) => {
      const sandbox = cloudSandboxes.get(req.params.sandboxId);
      if (!sandbox) {
        res.status(404).json({ error: "Sandbox not found" });
  ...
  app.post(
    "/engine/v1/sandboxes/:sandboxId/share-links",
    (req: express.Request, res: express.Response) => {
      const sandbox = cloudSandboxes.get(req.params.sandboxId);
      if (!sandbox) {
        res.status(404).json({ error: "Sandbox not found" });
  ...
  app.get(
    "/engine/v1/sandboxes/:sandboxId/logs",
    (req: express.Request, res: express.Response) => {
      const sandbox = cloudSandboxes.get(req.params.sandboxId);
      if (!sandbox) {
        res.status(404).json({ error: "Sandbox not found" });

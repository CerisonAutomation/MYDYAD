...
/** Schema for validating token refresh responses from oauth.dyad.sh */
const NeonTokenRefreshResponseSchema = z.object({
  accessToken: z.string().min(1, "accessToken is required"),
  refreshToken: z.string().min(1, "refreshToken is required"),
  expiresIn: z.number().positive("expiresIn must be positive"),
});
...
async function refreshNeonTokenOnce(): Promise<void> {
  ...
  try {
    ...
    const parsed = NeonTokenRefreshResponseSchema.safeParse(rawResponse);
    if (!parsed.success) {
      throw new DyadError(
        `Neon token refresh returned invalid data: ${parsed.error.issues.map((i) => i.message).join(", ")}`,
        DyadErrorKind.External,
      );
    }
    const {

...
/** Schema for validating token refresh responses from supabase-oauth.dyad.sh */
...
async function refreshSupabaseTokenOnce(): Promise<void> {
  ...
  try {
    ...
    const parsed = TokenRefreshResponseSchema.safeParse(rawResponse);
    if (!parsed.success) {
      throw new DyadError(
        `Supabase token refresh returned invalid data: ${parsed.error.issues.map((i) => i.message).join(", ")}`,
        DyadErrorKind.External,
      );
    }
    const {

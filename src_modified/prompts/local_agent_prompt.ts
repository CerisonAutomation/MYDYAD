...
/**
 * Escape XML special characters to prevent injection into XML-tagged prompt
 * blocks. User-controlled content (AI_RULES.md, themePrompt) is injected into
 * <ai_rules> blocks; without escaping, a crafted file could inject arbitrary
 * instruction blocks via </ai_rules> followed by new XML tags.
 */
...
export function constructLocalAgentPrompt(
  ...
  // XML-escape user content to prevent injection into <ai_rules> blocks.
  let prompt = basePrompt
    .replace("[[SERVER_LAYER]]", () => serverLayer)
    .replace("[[AI_RULES]]", () => escapeXml(aiRules ?? DEFAULT_AI_RULES));

  // Append theme prompt if provided (also XML-escape to prevent injection)
  if (themePrompt) {
    prompt += "\n\n" + escapeXml(themePrompt);
  }

...
export function ModelPicker() {
  ...
  const handleUnlockAllClick = () => {
    // Subscription upgrade removed — all models available with BYOK.
    setOpen(false);
  };
    ...
    <>
      <DropdownMenu open={open} onOpenChange={handleOpenChange}>
        ...
        <DropdownMenuContent className="w-[17rem]" align="start">
          {/* Trial user banner — hidden, no marketing */}
          {isTrial && (
            <>
          ...
          {/* Cloud models — all users can use all models */}
          {!isTrial &&
            (loading ? (
          ...
          {/* All models available — no unlock footer */}
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      >
        ...
        >
          ...
          {unlockTargetIsFreeModel ? (
            ...
            <>
              <DialogHeader>
                <DialogTitle>Use {unlockTarget?.model.displayName}</DialogTitle>
                <DialogDescription>
                  Configure a provider API key to use this model.
                </DialogDescription>
              </DialogHeader>

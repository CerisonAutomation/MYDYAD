...
export function NodePathSelector() {
  ...
  const checkNodeStatus = async () => {
    ...
    try {
      const status = await ipc.instructions.getNodejsStatus();
      setNodeStatus({
  ...
  useEffect(() => {
    return ipc.events.instructions.onManagedNodeInstallProgress((progress) => {
  ...
  const handleSelectNodePath = async () => {
    ...
    try {
      ...
      const result = await ipc.instructions.selectNodeFolder();
      if (result.path) {
        // Save the custom path to settings
        await updateSettings({ customNodePath: result.path });
        // Update the environment PATH
        await ipc.instructions.reloadEnvPath();
  ...
  const refreshNodeStatus = async () => {
    await queryClient.invalidateQueries({
      queryKey: queryKeys.instructions.nodejsStatus,
    });
    await fetchSystemPath();
    await checkNodeStatus();
  };
  ...
  const handleInstallManagedNode = async () => {
    ...
    try {
      await ipc.instructions.installManagedNode();
      await queryClient.invalidateQueries({
  ...
  const handleRemoveManagedNode = async () => {
    ...
    try {
      await ipc.instructions.removeManagedNode();

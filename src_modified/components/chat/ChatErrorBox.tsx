...
export function ChatErrorBox({
  ...
  const isTrialProUser = false; // BYOK: no trial

  if (error.includes("doesn't have a free quota tier")) {
    return (
      <ChatErrorContainer onDismiss={onDismiss}>
        {error} — switch to another model or configure a provider API key.
      </ChatErrorContainer>
    );
  }
  ...
  if (error.includes("LiteLLM Virtual Key expected")) {
    return (
      <ChatInfoContainer onDismiss={onDismiss}>
        <span>Invalid API key. Please check your provider settings.</span>
      </ChatInfoContainer>
    );
  }
  if (error.includes("ExceededBudget:")) {
    return (
      <ChatInfoContainer onDismiss={onDismiss}>
        <span>
          Budget exceeded. Switch to a different model or configure your own API
          key.
        </span>
      </ChatInfoContainer>
    );
  ...
  if (error.includes("FREE_AGENT_QUOTA_EXCEEDED")) {
    return (
      <ChatErrorContainer onDismiss={onDismiss}>
        Agent quota exceeded. Switch to Build mode or configure a provider API
        key.
      </ChatErrorContainer>
    );
  }
  ...
  if (isFreeModelQuotaError) {
    ...
    return (
      <ChatErrorContainer onDismiss={onDismiss}>
        <span>
          You have reached the {freeModelMessagesLimit}-message free model
          limit.{resetText} Switch to a different model.
        </span>
      </ChatErrorContainer>
    );
  }

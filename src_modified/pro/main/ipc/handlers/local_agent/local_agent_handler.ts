...
export async function handleLocalAgentStream(
  ...
  // BYOK users with any provider key can also use Agent mode
  if (
    !readOnly &&
    !planModeOnly &&
    !isDyadProEnabled(settings) &&
    !isBasicAgentMode(settings) &&
    !hasAnyProviderKey(settings)
  ) {
    const errorMessage =
      referencedApps.length > 0
        ? "Referencing other apps (@app:Name) in local-agent mode requires a provider API key. Please configure a provider in Settings → Providers."
        : "Agent v2 requires a provider API key. Please configure a provider in Settings → Providers.";
    ...
    while (!abortController.signal.aborted) {
      ...
      while (!abortController.signal.aborted) {
        ...
        try {
          ...
          const fullStream = streamResult.stream;

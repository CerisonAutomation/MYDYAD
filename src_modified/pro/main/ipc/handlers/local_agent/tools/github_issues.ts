...
Use for: Project management, bug tracking, feature requests.`,
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<dyad-github-issues op="${args.operation}">Loading...</dyad-github-issues>`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    if (args.operation === "list") {
      ...
      if (!args.issue_number) throw new Error("issue_number required for get");
      ghArgs = [
        "issue",
        "view",
    ...
    ctx.onXmlComplete(
      `<dyad-github-issues op="${args.operation}">${JSON.stringify(result, null, 2)}</dyad-github-issues>`,
    );

 ...
 */
export async function resolveDirectoryWithinAppPath(params: {
  appPath: string;
  directory: string;
}): Promise<string> {
  ...
  try {
    ...
    // If the target doesn't exist yet, fall back to the unresolved path.
    // This is safe because the structural check below still applies.
    realResolvedPath = resolvedPath;
  }

  let realAppPath: string;
  try {
    realAppPath = await fs.realpath(resolvedAppPath);
  } catch {
    realAppPath = resolvedAppPath;
  }

  const appForCheck = caseInsensitive ? realAppPath.toLowerCase() : realAppPath;
  const targetForCheck = caseInsensitive
    ? realResolvedPath.toLowerCase()
    : realResolvedPath;
  ...
  return pathImpl.relative(realAppPath, realResolvedPath);
}

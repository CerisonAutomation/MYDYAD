...
/**
 * Call Dyad Engine web fetch (original pattern)
 */
async function callEngineWebFetch(
  url: string,
...
/**
 * Call local web fetch (BYOK fallback)
 */
async function callLocalWebFetch(
  url: string,
): Promise<z.infer<typeof webFetchResponseSchema>> {
  const result = await localWebFetch(url, { maxLength: MAX_CONTENT_LENGTH });
...
export const webFetchTool: ToolDefinition<z.infer<typeof webFetchSchema>> = {
  ...
  execute: async (args, ctx) => {
    ...
    try {
      let result: z.infer<typeof webFetchResponseSchema>;

      // Use Dyad Engine if available, otherwise use local fetch
      // Falls back to local if engine auth fails (BYOK users without auto key)
      if (ctx.isDyadPro) {
        try {
          result = await callEngineWebFetch(args.url, ctx);
        } catch (engineError) {
          if (
            engineError instanceof DyadError &&
            engineError.kind === DyadErrorKind.Auth
          ) {
            logger.log("Engine auth failed, falling back to local web fetch");
            result = await callLocalWebFetch(args.url);
          } else {
            throw engineError;
          }
        }
      ...
      // Combine markdown from all pages (matching original behavior)

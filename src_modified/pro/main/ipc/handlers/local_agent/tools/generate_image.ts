...
const ImageGenerationApiResponseSchema = z.object({
  data: z.array(
    z.object({
...
/**
 * Call Dyad Engine image generation (original pattern)
 */
async function callEngineGenerateImage(
  prompt: string,
...
/**
 * Call local image generation (BYOK fallback)
 */
async function callLocalGenerateImage(
  prompt: string,
): Promise<z.infer<typeof ImageGenerationApiResponseSchema>["data"][number]> {
  const result = await localImageGeneration({
    prompt,
    width: 512,
    height: 512,
  });

  // Convert to original Dyad Engine response format
  return {
    url: result.url,
    b64_json: undefined,
  };
...
> = {
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    try {
      let imageData: z.infer<
        typeof ImageGenerationApiResponseSchema
      >["data"][number];

      // Use Dyad Engine if available, otherwise use local generation
      // Falls back to local if engine auth fails (BYOK users without auto key)
      if (ctx.isDyadPro) {
        try {
          imageData = await callEngineGenerateImage(args.prompt, ctx);
        } catch (engineError) {
          if (
            ...
            logger.log(
              "Engine auth failed, falling back to local image generation",
            );
            imageData = await callLocalGenerateImage(args.prompt);
          } else {
            throw engineError;
          }
        }

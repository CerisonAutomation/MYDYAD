...
Returns: Author, date, commit hash for each line.`,
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<dyad-git-blame file="${args.file_path}">Blaming...</dyad-git-blame>`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    try {
      ...
      ctx.onXmlComplete(
        `<dyad-git-blame file="${args.file_path}" lines="${result.length}">${JSON.stringify(result, null, 2)}</dyad-git-blame>`,
      );

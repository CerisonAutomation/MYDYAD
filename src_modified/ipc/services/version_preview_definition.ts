...
function reconcileRestore(
  ...
  // If recovery conditions aren't met, close cleanly instead of showing error.
  // The user's dev environment doesn't need version-restore recovery.
  if (current.type === "restore-recovery-required") {
    return CLOSED_STATE;
  }
      ...
      state =
        !ownsCheckout || reachedSafeBranch
          ...
          : {
              ...
              error: {
                message: "Repository needs repair.",
              },
            };
    } else if (
      current.type === "restoring" ||

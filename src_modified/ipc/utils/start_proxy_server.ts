...
export async function startProxy(
  ...
  // Resolve proxy_server.js — in production __dirname is the app/ directory
  const proxyPath = path.resolve(__dirname, "worker", "proxy_server.js");
  const proxyPathAlt = path.resolve(
    __dirname,
    "..",
    "..",
    "worker",
  ...
  const finalProxyPath = fs.existsSync(proxyPath) ? proxyPath : proxyPathAlt;
  logger.log("Proxy worker path:", finalProxyPath);

  const worker = new Worker(finalProxyPath, {
    workerData: {
      targetOrigin,
      port,
      fallbackPortStart,

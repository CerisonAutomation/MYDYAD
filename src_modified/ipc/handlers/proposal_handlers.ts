...
function cleanupExpiredCacheEntries() {
  ...
  // Enforce size cap after TTL cleanup
  if (codebaseTokenCache.size > MAX_CACHE_ENTRIES) {
    const entries = [...codebaseTokenCache.entries()].sort(
      (a, b) => a[1].timestamp - b[1].timestamp,
    );
    const toRemove = entries.slice(
      0,
      codebaseTokenCache.size - MAX_CACHE_ENTRIES,
    );

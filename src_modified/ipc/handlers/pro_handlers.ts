...
export function registerProHandlers() {
  ...
  // Subscription status stubbed — no Dyad hosted engine in BYOK build.
  typedHandle(systemContracts.getSubscriptionStatus, async () => {
    return null;
  });

  // Billing action stubbed — no subscription billing in BYOK build.
  typedHandle(systemContracts.openBillingAction, async () => {
    // No-op: subscription billing removed.
  });

...
const MAX_HELP_SESSIONS = 10;
const helpSessions = new Map<string, HelpMessage[]>();
const activeHelpStreams = new Map<string, AbortController>();

/** Evict the oldest session if the map exceeds the cap. */
function evictOldestHelpSession(): void {
  if (helpSessions.size <= MAX_HELP_SESSIONS) return;
  const oldest = helpSessions.keys().next().value;
  if (oldest !== undefined) {
    helpSessions.delete(oldest);
    const controller = activeHelpStreams.get(oldest);
    if (controller) {
      controller.abort();
      activeHelpStreams.delete(oldest);
    }
  }
}
...
export function registerHelpBotHandlers() {
  ...
  createTypedHandler(helpContracts.start, async (event, params) => {
    ...
    try {
      ...
      // this is required.
      const fullStream = stream.stream;

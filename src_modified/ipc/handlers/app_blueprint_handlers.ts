...
/** Evict the oldest blueprint if the store exceeds the cap. */
function evictOldestBlueprint(): void {
  if (appBlueprintStore.size <= MAX_BLUEPRINTS) return;
  const oldest = appBlueprintStore.keys().next().value;
  if (oldest !== undefined) {
    appBlueprintStore.delete(oldest);
  }
}

...
> {
  console.log("[PROVIDER-DEBUG] getLanguageModelProviders called!");
  // Fetch custom providers from the database
  let customProvidersDb: any[] = [];
  try {
    console.log("[DB-DEBUG] Attempting DB query for custom providers...");
    const result = await db.select().from(languageModelProvidersSchema);
    customProvidersDb = result;
    console.log(
      "[DB-DEBUG] Custom providers from DB:",
      customProvidersDb.length,
      "rows",
    );
    for (const cp of customProvidersDb) {
      console.log("[DB-DEBUG] DB provider:", cp.id, cp.name);
    }
  } catch (e) {
    console.error("[DB-DEBUG] DB query FAILED:", e);
    console.error(
      "[DB-DEBUG] Error:",
...
export async function getLanguageModels({
  ...
  const provider =
    ...
    allProviders.find((p) => p.id === `custom::${providerId}`) ??
    allProviders.find(
      (p) =>

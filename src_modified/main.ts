...
function resolveLocalGitDirectory() {
  ...
  // Packaged app: try extraResource first, then fall back to node_modules
  const extraResourceGit = path.join(process.resourcesPath, "git");
  if (fs.existsSync(path.join(extraResourceGit, "bin", "git"))) {
    return extraResourceGit;
  }
  // Fallback: git is in the app bundle's node_modules
  return path.join(app.getAppPath(), "node_modules", "dugite", "git");
}
...
const createWindow = ({
  ...
  // Fix: Forge injects localhost but Vite listens on 127.0.0.1 (IPv4)
  const allowedDevServerUrl =
    MAIN_WINDOW_VITE_DEV_SERVER_URL?.replace(/localhost/g, "127.0.0.1") ||
    undefined;
  ...
  if (!app.isPackaged && MAIN_WINDOW_VITE_DEV_SERVER_URL) {
    // Fix: Forge injects localhost but Vite listens on 127.0.0.1
    const devUrl = MAIN_WINDOW_VITE_DEV_SERVER_URL.replace(
      /localhost/g,
      "127.0.0.1",
    );
    initialLoad = browserWindow.loadURL(devUrl);
  } else {

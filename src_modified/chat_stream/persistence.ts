...
const MAX_INTENT_RECORDS = 500;
const intentRecords = new Map<string, IntentRecord>();
const queues = new Map<number, QueueAggregate>();

/** Evict the oldest intent records if the map exceeds the cap. */
function evictOldestIntents(): void {
  if (intentRecords.size <= MAX_INTENT_RECORDS) return;
  const toRemove = intentRecords.size - MAX_INTENT_RECORDS;
  let removed = 0;
  for (const key of intentRecords.keys()) {
    if (removed >= toRemove) break;
    const record = intentRecords.get(key);
    // Only evict terminal (completed/failed) records, not active ones
    if (record?.recovery === "terminal" || record?.acceptance === "rejected") {

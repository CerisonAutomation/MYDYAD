...
function injectHTML(buf) {
  ...
  // Resolve the parent (Electron shell) origin so injected shims can postMessage
  // to a concrete target instead of "*", preventing credential leakage to any
  // page that frames the preview.
  const escapedParentOrigin = (rememberedOrigin || "http://127.0.0.1:5173")
    .replace(/\\/g, "\\\\")
    .replace(/'/g, "\\'");
  ...
  if (!legacyAppWithShim) {
    ...
    if (dyadShimContent) {
      scripts.push(
        `<script>var DYAD_PARENT_ORIGIN='${escapedParentOrigin}';</script><script>${dyadShimContent}</script>`,
      );
    } else {
      scripts.push(
  ...
  if (dyadScreenshotClientContent) {
    scripts.push(
      `<script>var DYAD_PARENT_ORIGIN='${escapedParentOrigin}';</script><script>${dyadScreenshotClientContent}</script>`,
    );
  } else {
    scripts.push(
  ...
  if (dyadSwRegisterContent) {
    scripts.push(
      `<script>var DYAD_PARENT_ORIGIN='${escapedParentOrigin}';</script><script>${dyadSwRegisterContent}</script>`,
    );
  } else {
    scripts.push(

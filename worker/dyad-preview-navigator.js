...
(function () {
  ...
  async function takeScreenshot(opts) {
    ...
    return new Promise((resolve) => {
      ...
      const handler = (e) => {
        if (
          e.data?.source === "dyad-preview-navigator" &&
          e.data?.reqId === reqId
        ) {
  ...
  function pageInfo() {
    return {
      ...
      data: {
        ...
        errors: [
          ...document.querySelectorAll("[data-nextjs-error], .next-error-h1"),
  ...
  window.addEventListener("message", async (event) => {
    ...
    switch (method) {
      case "screenshot":
        result = await takeScreenshot(args?.[0] || {});
        break;
      case "navigate":
        result = navigate(args?.[0]);
        break;
      case "currentUrl":
        result = currentUrl();
        break;
      case "click":
        ...
        break;
      case "typeText":
        result = typeText(args?.[0], args?.[1]);
        break;
      case "getText":
        result = getText(args?.[0]);
        break;
      case "queryAll":
        result = queryAll(args?.[0]);
        break;
      case "eval":
        result = evalJs(args?.[0]);
        break;
      case "pageInfo":
        result = pageInfo();
        break;

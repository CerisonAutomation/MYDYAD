...
const SHARED_FILES_CACHE_MAX = 50;

function cacheSharedFiles(key: string, value: CachedSharedFiles): void {
  // Evict oldest entry if at capacity (Map preserves insertion order)
  if (sharedFilesCache.size >= SHARED_FILES_CACHE_MAX) {
    const oldestKey = sharedFilesCache.keys().next().value;
    if (oldestKey !== undefined) sharedFilesCache.delete(oldestKey);
  }
  sharedFilesCache.set(key, value);
}
...
async function getSharedFiles(appPath: string): Promise<CachedSharedFiles> {
  ...
  cacheSharedFiles(sharedDirectory, result);
  return result;
}

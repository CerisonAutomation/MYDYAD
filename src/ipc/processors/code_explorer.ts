...
function scheduleIdleKillIfIdle(): void {
  ...
  idleTimer = setTimeout(() => {
    ...
    // Clear module-level caches that are no longer relevant once the
    // worker process (and all its indices) is destroyed.
    lastCrashAtByKey.clear();
    unavailableKeys.clear();
    typeScriptFingerprintByAppPath.clear();

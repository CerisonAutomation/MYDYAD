...
export async function getModelClient(
  ...
  // Dyad hosted engine removed — all calls go directly to BYOK providers.
    ...
    for (const autoModelAlias of AUTO_MODEL_ALIASES) {
      ...
      const apiKey = getProviderApiKeyForRequest(
        settings.providerSettings?.[resolvedModel.providerId]?.apiKey?.value ||
          // For custom providers, envVarName IS the actual API key
          (providerInfo?.type === "custom" && envVarName
            ? envVarName
            : // For builtin providers, envVarName is an env var name
              envVarName
  ...
  const apiKey =
    providerId === "azure"
      ...
      : getProviderApiKeyForRequest(
          // Priority 1: explicit apiKey in providerSettings
          settings.providerSettings?.[model.provider]?.apiKey?.value ||
            // Priority 2: for custom providers, envVarName IS the actual API key
            (providerConfig.type === "custom" && providerConfig.envVarName
              ? providerConfig.envVarName
              : // Priority 3: for builtin providers, envVarName is an env var name
                providerConfig.envVarName
  ...
  switch (providerId) {
    ...
    default: {
      ...
      if (providerConfig.type === "custom") {
        ...
        // Strip trailing /chat/completions if present — AI SDK appends it

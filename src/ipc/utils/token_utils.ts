 ...
 *
 * For very large context windows (>500k), the cap scales to 85% of the
 * context window so we don't waste most of the available context.
 */
export function getCompactionThreshold(
  ...
  // For large context windows, scale the cap to use more of the available
  // context. At 1M tokens, this gives ~850k instead of the old 250k.
  const cap =
    contextWindow > 500_000
      ? Math.max(baseCap, Math.floor(contextWindow * 0.85))
      : baseCap;

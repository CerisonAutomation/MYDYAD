...
describe("packaged renderer route coverage", () => {
  it("trusts every static route registered with TanStack Router", () => {
    ...
    // Recursively collect all non-dynamic route paths from the route tree.
    const collectPaths = (node: any): string[] => {
      const paths: string[] = [];
      if (node?.options?.path && !node.options.path.includes("$")) {
        paths.push(node.options.path);
      }
    ...
    const staticRoutePaths = collectPaths((router as any).routeTree);

...
export function registerShellHandlers() {
  ...
  handle("show-item-in-folder", async (_event, fullPath: string) => {
    ...
    // Security: only allow showing files within .dyad/media subdirectories
    // or the dyad-apps directory tree.
    if (!isFileWithinAnyDyadMediaDir(fullPath)) {
      throw new DyadError(
        "Can only show files within .dyad/media directories.",
    ...
    logger.debug("Showed item in folder:", resolvedPath);
  });

...
type ForceClosePayload = Parameters<
  Parameters<typeof ipc.events.instructions.onForceCloseDetected>[0]
>[0];
type TelemetryPayload = Parameters<
  Parameters<typeof ipc.events.instructions.onTelemetryEvent>[0]
>[0];
...
export function registerEarlyRendererEvents(): void {
  ...
  ipc.events.instructions.onForceCloseDetected((payload) => {
    earlyForceCloseEvents.emit(payload);
  });
  ipc.events.instructions.onTelemetryEvent((payload) => {

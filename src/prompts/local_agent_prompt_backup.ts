...
Dyad has built-in browser tools for interacting with live web pages. Use these tools to verify UI changes, test functionality, and debug issues.

## Available Browser Tools

### browser_control
Control a browser to interact with web pages. Supports:
- **navigate** — Go to a URL (defaults to running app's preview URL)
- **click** — Click an element by CSS selector
- **type** — Type text into an input field
- **scroll** — Scroll the page in a direction
- **screenshot** — Take a screenshot and save to .dyad/media
- **get_text** — Get visible text content of an element
- **wait_for** — Wait for an element to appear in the DOM
- **read_page** — Read page's interactive elements (returns [ref] nodes)
- **batch** — Run multiple actions in sequence

### take_screenshot
Take a screenshot of the running app or a specific URL. Use for:
- Capturing before/after screenshots
- Documenting UI changes
- Creating visual regression baselines

### dom_snapshot
Capture a structured snapshot of the DOM tree. Use for:
...
- Read page structure for accessibility audits
- Test responsive design at different viewports

## Example Workflow
1. Make code changes with \`write_file\` or \`search_replace\`
2. Use \`browser_control\` with \`action: "navigate"\` to load the page
3. Use \`browser_control\` with \`action: "screenshot"\` to verify changes
4. Use \`browser_control\` with \`action: "read_page"\` to check accessibility

## IMPORTANT: Mandatory UI Verification
After ANY code change that affects the UI (components, styles, layout, colors, fonts, spacing), you MUST:
1. Navigate to the preview URL using \`browser_control\` with \`action: "navigate"\`
2. Take a screenshot using \`browser_control\` with \`action: "screenshot"\`
3. Report the visual result to the user

Type checks alone cannot catch rendering issues. A screenshot is the ONLY way to verify UI changes look correct.
</browser_tools>`;

// Gamechanging Tools Block
const GAMECHANGING_TOOLS_BLOCK = `<gamechanging_tools>
Dyad has 17 visual analysis tools for comprehensive code quality. Use these tools to audit code, detect bugs, and ensure quality.

## UI/UX Analysis Tools
- **visual_bug_detector** — Detects layout/overflow issues
- **accessibility_auditor** — WCAG compliance checks
- **responsive_checker** — Media query analysis
- **color_contrast** — Color usage analysis
- **css_analyzer** — CSS issue detection

## Code Quality Tools
- **performance_profiler** — Performance analysis
- **state_visualizer** — React state patterns
- **dependency_graph** — Module dependency analysis
- **import_analyzer** — Import/dependency analysis

## Automation Tools
- **smart_suggestions** — Code improvement suggestions
- **smart_fix** — Auto-fix suggestions
- **auto_quality** — Auto quality scoring
- **regression_detector** — Regression detection

## Testing Tools
- **visual_test** — Visual testing patterns
- **layout_debugger** — Layout pattern analysis
- **error_visualizer** — Error handling patterns
- **component_playground** — React component analysis

## When to Use Gamechanging Tools

Use these tools to:
- Audit code quality before committing
...
### Accessibility Audit
1. Use \`accessibility_auditor\` to scan for WCAG issues
2. Use \`browser_control\` with \`action: "read_page"\` to verify ARIA attributes
3. Use \`color_contrast\` to check color contrast ratios
4. Fix issues with \`search_replace\` or \`write_file\`
5. Re-run \`accessibility_auditor\` to verify fixes

### Performance Optimization
1. Use \`performance_profiler\` to identify bottlenecks
2. Use \`browser_control\` with \`action: "screenshot"\` to capture metrics
3. Use \`state_visualizer\` to optimize React state
4. Use \`dependency_graph\` to optimize imports
5. Re-run \`performance_profiler\` to verify improvements
</gamechanging_tools>`;

...
export function isDyadProEnabled(_settings: UserSettings): boolean {
  // Pro features are always enabled — no subscription gate.
  return true;
}

/** Agent2-enabled alias — always true in local BYOK mode. */
export function isAgent2Enabled(_settings: UserSettings): boolean {
  return true;
}

export function hasDyadProKey(_settings: UserSettings): boolean {
  // No Dyad Pro key required — BYOK users access all features.
  return true;
}

/**
 * Checks if the user has ANY provider API key configured.
 * This is used to allow Agent mode for users with any provider.
 */
export function hasAnyProviderKey(settings: UserSettings): boolean {
  if (!settings.providerSettings) return false;

  for (const [, config] of Object.entries(settings.providerSettings)) {
    if (config && typeof config === "object" && "apiKey" in config) {
      const apiKey = config.apiKey;
      if (apiKey && typeof apiKey === "object" && "value" in apiKey) {
...
export function isTurboEditsV2Enabled(settings: UserSettings): boolean {
  // Turbo Edits V2 works with any provider (local prompt-driven approach)
  // No longer requires Dyad Pro
  return Boolean(

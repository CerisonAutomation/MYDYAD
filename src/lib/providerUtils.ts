...
export function isProviderSetup(
  ...
  // Check provider data for env var name (for cloud providers)
  // Also check custom:: prefixed IDs — custom providers store the actual API key
  // in envVarName (not an env var name), and take priority over builtins.
  const providerInfo =
    providerData?.find((p) => p.id === `custom::${provider}`) ??
    providerData?.find(
      (p) => p.id === provider || p.id === `custom::${provider}`,
    );
  if (providerInfo?.envVarName && envVars[providerInfo.envVarName]) {
    return true;
  }

  // For custom providers, envVarName stores the actual API key directly
  // (not an env var name), so if it exists the provider is configured.
  if (providerInfo?.type === "custom" && providerInfo?.envVarName) {
    return true;
  }

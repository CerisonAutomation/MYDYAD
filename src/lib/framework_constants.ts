...
export function getFrameworkDevPortStrategy(
  ...
  // If the dev script already contains a port flag (-p or --port),
  // use the PORT env var instead of appending --port to avoid conflicts
  // (e.g. `next dev -p ${PORT:-3000} --port 32102` would break).
  const hasPortFlag = lower.includes(" -p ") || lower.includes(" --port ");
  if (hasPortFlag) return "env";

...
export async function runCurrentArm(params: {
  ...
  const { text, steps } = await finalTextOrNudge({
    model: params.model,
    instructions: CURRENT_ARM_SYSTEM(params.appName),
...
export async function runSubagentArm(params: {
  ...
  const { text: reportText, steps } = await finalTextOrNudge({
    model: params.model,
    instructions: EXPLORER_SYSTEM(params.appName),
  ...
  const answerResult = await generateText({
    model: params.model,
    instructions: SUBAGENT_ANSWER_SYSTEM(params.appName),
...
export async function runControlArm(params: {
  ...
  const result = await generateText({
    model: params.model,
    instructions: CONTROL_SYSTEM(params.appName),
    prompt: params.question,
    maxRetries: 5,
  });
...
export async function judgeAnswer(params: {
  ...
  const result = await generateText({
    ...
    maxRetries: 5,
    instructions: `You grade an AI assistant's answer to a user's question about their own prior conversations. You get the question, the ground-truth ideal answer, a grading rubric, and the assistant's answer.

...
function formatSettingsLines(
  ...
  return [
    ...
    `- Local Agent Mode: ${settings.enableDyadPro ?? "n/a"}`,
...
function openGitHubIssue(params: {
  ...
  labels: string[];
  body: string;
  isAgent2User: unknown;
}) {
  const labels = [...params.labels];
  if (params.isAgent2User) labels.push("agent2");
  const qs = new URLSearchParams({
    title: params.title,
    labels: labels.join(","),
    body: params.body,
  });
  ipc.instructions.openExternalUrl(`${GITHUB_ISSUES_BASE}?${qs.toString()}`);
}
...
export function HelpDialog() {
  ...
  const isAgent2User = settings?.providerSettings?.["auto"]?.apiKey?.value;
  ...
  const handleReportBug = async () => {
    setIsLoading(true);
    try {
      const debugInfo = await ipc.instructions.getSystemDebugInfo();
      const body = `\
...
`;
      ...
      ipc.instructions.openExternalUrl(GITHUB_ISSUES_BASE);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      await ipc.instructions.uploadToSignedUrl({
        url: uploadUrl,
        contentType: "application/json",
        data: debugBundle,
      });
      ...
      <div className="flex flex-col w-full mt-4 space-y-5">
        ...
        {isAgent2User ? (
          ...
          <Button
            variant="outline"
            onClick={() =>
              ipc.instructions.openExternalUrl("https://www.dyad.sh/docs")
            }
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          <div className="border rounded-lg p-4 space-y-3 relative">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold">AI Provider issues</span>
            </div>
      ...
      >
        ...
        <div className="space-y-2 overflow-y-auto flex-grow mt-4">
          ...
          <ReviewDetailsSection title="System Information" mono={false}>
            <p>Dyad Version: {debugBundle.instructions.dyadVersion}</p>
            <p>Platform: {debugBundle.instructions.platform}</p>
            <p>Architecture: {debugBundle.instructions.architecture}</p>
            <p>
              Node Version:{" "}
              {debugBundle.instructions.nodeVersion || "Not available"}
            </p>
          </ReviewDetailsSection>

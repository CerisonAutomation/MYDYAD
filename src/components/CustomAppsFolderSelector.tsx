...
export function CustomAppsFolderSelector() {
  ...
  const handleSelectCustomAppsFolder = async () => {
    ...
    try {
      ...
      const result = await ipc.instructions.selectCustomAppsFolder();
      if (result.path) {
        // Save the custom path to settings
        await ipc.instructions.setCustomAppsFolder(result.path);
  ...
  const handleResetToDefault = async () => {
    try {
      ...
      await ipc.instructions.setCustomAppsFolder(null);
  ...
  const fetchCustomAppsFolder = async () => {
    try {
      const { path, isPathAvailable, isPathDefault } =
        await ipc.instructions.getCustomAppsFolder();

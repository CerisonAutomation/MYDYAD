...
export function ProviderSettingsPage({ provider }: ProviderSettingsPageProps) {
  ...
  const handleSaveKey = async (
    ...
    try {
      ...
      // Check if this is the first time user is setting up Agent2
      ...
      // Refetch user budget when Agent2 key is saved
      if (isDyad) {
  ...
  // --- Toggle Agent2 Handler ---
  const handleToggleAgent2 = async (enabled: boolean) => {
    setIsSaving(true);
    try {
      await updateSettings({
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {isDyad && !settingsLoading && (
            <div className="mt-6 flex items-center justify-between p-4 bg-(--background-lightest) rounded-lg border">
              <div>
                <h3 className="font-medium">Local Agent Mode</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  All features enabled — use your own API keys
                </p>
              </div>
              <Switch
                aria-label="Enable Local Agent Mode"
                checked={settings?.enableDyadPro}
                onCheckedChange={handleToggleAgent2}
                disabled={isSaving}
              />
            </div>
          )}

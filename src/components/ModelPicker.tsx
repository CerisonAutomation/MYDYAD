...
export function ModelPicker() {
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    ...
    if (nextOpen) {
      posthog.capture("model-picker:open", {
        isAgent2: settings ? isAgent2Enabled(settings) : false,
      });
    }
  };
  ...
  const agent2Enabled = settings ? isAgent2Enabled(settings) : false;
  // Ollama Models Hook
  const {
    models: ollamaModels,
    ...
    !loading && modelsByProviders && modelsByProviders["auto"]
      ? modelsByProviders["auto"].filter((model) => {
          if (model.apiName === FREE_PRO_MODEL_NAME) {
            return agent2Enabled && !isTrial && !isLoadingTrialStatus;
          }
          if (settings && !agent2Enabled && model.apiName === "value") {
            return false;
          }
          if (settings && agent2Enabled && model.apiName === "free") {
            return false;
          }
          return true;
        })
  ...
  const isModelLocked = (providerId: string) => {
    if (settingsLoading || agent2Enabled || providerId === "auto") {
      return false;
    }
    const provider = providers?.find((p) => p.id === providerId);
  ...
  const renderProviderSubmenu = (
    ...
    const visibleModels = models.filter((model) => {
      if (agent2Enabled && isFreeOpenRouterModelName(model.apiName)) {
        return false;
      }
      return true;
    });
  ...
  return (
    <>
      <DropdownMenu open={open} onOpenChange={handleOpenChange}>
        <div className="flex items-center">
          ...
          <button
            ...
            className="ml-1 size-6 flex items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors"
            title="Search models (Ctrl+K)"
          >
      ...
      {/* Fuse.js-powered search dialog */}
      <ModelSearchDialog
        open={searchOpen}
        onOpenChange={setSearchOpen}
        onSelect={({ name, provider, customModelId }) => {
          void onModelSelect({
            model: { name, provider, customModelId },
          });
        }}
        ...
        agent2Enabled={agent2Enabled}
        isProviderSetup={isProviderSetup}
        currentEffortLevel={selectedEffortLevel}
        onOpenSettings={() => {
          navigate({
            to: providerSettingsRoute.id,

...
/** Scale the long-context threshold for large context windows. */
function getLongContextThreshold(contextWindow: number): number {
  // For windows >500k, use 20% of context as the threshold
  if (contextWindow > 500_000) {
    return Math.floor(contextWindow * 0.2);
  }
...
export function shouldShowContextLimitBanner({
  ...
  if (totalTokens > getLongContextThreshold(contextWindow)) {
    return true;
  }

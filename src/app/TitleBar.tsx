...
export const TitleBar = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "dyad-pro-return") {
        ...
        // Refetch user budget when Agent2 mode is set via deep link
  ...
  const isAgent2Enabled = Boolean(settings?.enableDyadPro);
    ...
    <>
      <div className="@container z-11 w-full h-[calc(var(--layout-title-bar-offset)+1px)] pt-1 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        <div className="flex items-center shrink-0">
          ...
          {isDyadPro && <DyadProButton isAgent2Enabled={isAgent2Enabled} />}
        </div>
...
export function DyadProButton({
  ...
  return (
    <Button
      ...
      size="sm"
    >
      {isAgent2Enabled ? "Agent2" : "Agent2 (off)"}
      {userBudget && isAgent2Enabled && (

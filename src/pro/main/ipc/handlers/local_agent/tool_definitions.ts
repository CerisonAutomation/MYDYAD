...
import { codeComplexityTool } from "./tools/code_complexity";
import { apiGeneratorTool } from "./tools/api_generator";
...
export const TOOL_DEFINITIONS: readonly ToolDefinition[] = [
  ...
  // Advanced features tools (work with any provider)
  smartContextTool,
  localLazyEditsTool,
  localTranscribeTool,
  withAnalysisGuard(hotspotsTool),
  // Code quality analysis tools (from repo-intel) — guarded for concurrency
  withAnalysisGuard(codeSmellsTool),
  withAnalysisGuard(deadCodeTool),
  withAnalysisGuard(securityScanTool),
  withAnalysisGuard(complexityTool),
  withAnalysisGuard(testGapsTool),
  withAnalysisGuard(apiExtractTool),
  withAnalysisGuard(depAuditTool),
  withAnalysisGuard(contextOptimizeTool),
  // Additional analysis tools (inspired by Lovable MCP) — guarded
  withAnalysisGuard(perfAuditTool),
  withAnalysisGuard(architectureMapTool),
  withAnalysisGuard(repoPulseTool),
  withAnalysisGuard(licenseCheckTool),
  // Action and review tools — guarded
  withAnalysisGuard(actionPlanTool),
  withAnalysisGuard(onboardingBriefTool),
  withAnalysisGuard(diffImpactTool),
  withAnalysisGuard(reviewPrTool),
  withAnalysisGuard(testPlanTool),
  // MCP-inspired tools (Serena, Sequential Thinking, Crawl4AI, Code Context)
  withAnalysisGuard(symbolOpsTool),
  sequentialThinkingTool,
  crawl4aiTool,
  codeContextTool,
  // Static Analysis tools (Semgrep, CodeQL) — guarded
  withAnalysisGuard(semgrepScanTool),
  withAnalysisGuard(codeqlScanTool),
  // Game-changing tools (AI-powered automation) — guarded
  withAnalysisGuard(autoRefactorTool),
  dependencyUpdaterTool,
  withAnalysisGuard(testGeneratorTool),
  withAnalysisGuard(documentationGeneratorTool),
  apiGeneratorTool,
  withAnalysisGuard(securityPentestTool),
  // Prompt engineering tools (DSPy/Promptfoo-inspired)
  promptOptimizerTool,
  promptTesterTool,
  promptBenchmarkerTool,
  promptRedTeamTool,
  // Git and team tools (repo-features-mcp-pro)
  busFactorTool,
  typeSafetyTool,
  gitBlameTool,
  gitBranchesTool,
  ...
  withAnalysisGuard(codeReviewBotTool),
  withAnalysisGuard(codeContextGraphTool),
  // Multifetch, Batch, Thinking, Provider Router
  multifetchTool,
  batchExecutorTool,
  thinkingChainTool,
  providerRouterTool,
  // 17 Gamechanging Visual Tools — heavy ones guarded
  withAnalysisGuard(visualBugDetectorTool),
  withAnalysisGuard(accessibilityAuditorTool),
  withAnalysisGuard(responsiveCheckerTool),
  withAnalysisGuard(performanceProfilerTool),
  withAnalysisGuard(stateVisualizerTool),
  withAnalysisGuard(dependencyGraphTool),
  withAnalysisGuard(smartSuggestionsTool),
  withAnalysisGuard(layoutDebuggerTool),
  withAnalysisGuard(errorVisualizerTool),
  withAnalysisGuard(importAnalyzerTool),
  withAnalysisGuard(componentPlaygroundTool),
  withAnalysisGuard(smartFixTool),
  withAnalysisGuard(visualTestTool),
  withAnalysisGuard(regressionDetectorTool),

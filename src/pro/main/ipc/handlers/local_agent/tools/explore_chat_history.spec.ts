...
describe("exploreChatHistoryTool contract", () => {
  ...
  it("is disabled in free-model mode even for Pro", () => {
    expect(
      exploreChatHistoryTool.isEnabled!(
        makeAgentContext({ isDyadPro: true, freeModelMode: true }),
      ),
    ).toBe(false);
  });
});
...
describe("tool exposure via shouldIncludeTool", () => {
  ...
  it("is excluded in free-model mode even for Pro (engine-backed)", () => {
    const ctx = makeAgentContext({ isDyadPro: true, freeModelMode: true });
    expect(shouldIncludeTool(exploreChatHistoryTool, ctx)).toBe(false);
  });
  ...
  it("keeps search_chats for non-Pro, and for Pro when the explorer is filtered out", () => {
    ...
    const pro = makeAgentContext({ isDyadPro: true, freeModelMode: true });
    expect(shouldIncludeTool(searchChatsTool, pro)).toBe(true);
  });
});

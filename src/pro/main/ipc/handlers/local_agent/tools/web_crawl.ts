...
import { crawl4aiTool } from "./crawl4ai";
...
**Use the markdown below as your reference** to understand the page structure, content, and layout of the website.
...
export const webCrawlTool: ToolDefinition<z.infer<typeof webCrawlSchema>> = {
  ...
  execute: async (args, ctx) => {
    ...
    try {
      // Use local crawl4ai implementation
      const crawlResult = await crawl4aiTool.execute(
        {
          url: args.url,
      ...
      if (!crawlResult || typeof crawlResult !== "string") {
        throw new DyadError(
      ...
      const messageContent: Parameters<typeof ctx.appendUserMessage>[0] = [
        ...
        {
          type: "text",
          text: formatSnippet("Markdown snapshot:", crawlResult, "markdown"),
        },
      ];
      ...
      throw new DyadError(
        `Web crawl failed: ${error instanceof Error ? error.message : String(error)}`,
        DyadErrorKind.External,
      );
    }
  },

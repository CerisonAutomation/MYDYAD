...
export async function drainChatSearchIndexOnce(): Promise<void> {
  await drainDirtyChats();
  let processed: number;
  let totalProcessed = 0;
  const MAX_TOTAL = 10_000;
  const MAX_DURATION_MS = 5_000; // 5 seconds max per drain cycle
  ...
  do {
    ...
    if (totalProcessed > MAX_TOTAL) {
      logger.error(
        `drainChatSearchIndexOnce processed ${totalProcessed} messages — aborting to prevent CPU spin`,
      );
      return;
    }
    if (Date.now() - startTime > MAX_DURATION_MS) {
      logger.warn(
        `drainChatSearchIndexOnce exceeded ${MAX_DURATION_MS}ms — yielding to event loop`,
      );
      return;
    }
    if (processed > 0) {
 ...
 *
 * Yields to the event loop between batches so a large backlog of dirty chats
 * cannot monopolize the main process and cause a CPU-spin freeze.
 */
async function drainDirtyChats(): Promise<void> {
  ...
  // Safety limit: prevent infinite loop if triggers re-insert rows.
  // 100 iterations × 50 batch = 5000 chats max, far exceeds any realistic backlog.
  const MAX_ITERATIONS = 100;
  let iterations = 0;
  for (;;) {
    if (++iterations > MAX_ITERATIONS) {
      logger.error(
        `drainDirtyChats exceeded ${MAX_ITERATIONS} iterations — aborting to prevent CPU spin`,
      );
      return;
    }
    const rows = client
    ...
    // Yield between batches so a large backlog cannot monopolize the
    // main process event loop and cause a CPU-spin freeze.
    await new Promise((resolve) => setImmediate(resolve));
  }
}

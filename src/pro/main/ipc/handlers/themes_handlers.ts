...
import { type FilePart, type TextPart, streamText } from "ai";
...
Modern theme extracted from website for testing.
      ...
      if (!apiKey) {
        throw new DyadError(
          "Agent2 mode requires an API key. Configure your provider in Settings.",
          DyadErrorKind.Auth,
        );
      }
      ...
      // Agent2 mode: check if Dyad engine is available for web crawl
      const crawlEngineUrl = getDyadEngineBaseUrl();
      if (!crawlEngineUrl) {
        throw new DyadError(
          "Web crawl for theme generation requires a backend API. " +
            "In Agent2 mode, upload images directly instead of using a URL.",
          DyadErrorKind.Validation,
        );
      }
      ...
      try {
        crawlResponse = await fetch(`${crawlEngineUrl}/tools/web-crawl`, {
          method: "POST",
          headers: {
